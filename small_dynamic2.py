...
def sys(t, x, params):
    ...
    CORTI = cort_i(t, x, v)
    CORTK = cort_k(t, x, v)
    CORTISONEP = Cortisonep(t, x, v)
    CORTISONEK = Cortisonek(t, x, v)

    return [CRH, ACTH, CORTP, CORTI, CORTK, CORTISONEP, CORTISONEK,
...
# Cort Plasma
params.add('Dcort_pi', value=10.8, min=5, max=1e5)
params.add('Dcort_pk', value=9.0, min=5, max=20)
...
params.add('ucp', value=0.0016, min= 1e-3, max=1)
params.add('k3', expr=k3expr)
...
def residual_galluci(ps, fun, t, ts, data, x0):
    ...
    # Interstital must be within 15% of free
    fi = (0.85 * z.y.T[:,2]/ps['v_p'].value -
        z.y.T[:,3]/ps['v_i'].value).ravel()
    cort = toCortT_ugdl(z.y.T, ps) 
    model = np.column_stack((acth, cort))
    dataarr= (model - data).ravel()
    return np.concatenate([dataarr, fi])
...
def main(params, ts):
    ...
    for i in range(num_runs):
        ...
        final = male_res.residual[:12].reshape(gall_male.shape) + gall_male
        ...
        # Plot with from solver
        ts2 = np.linspace(0,120,120)
        m_sol = g(sys,[ts[0], ts[-1]] , ts2, x0_m, male_res.params)
        z = m_sol.y.T


        def cort_p2(t, x, v):
            arr = np.zeros((x.shape[0],8))
            # stimulation
             ...
             * v['tonano']
            # albumin off binding
            + v['koffA'] * (x[:,15]/v['v_p']) * (v['MWCort']/v['MWAC'])) * v['v_p']
            # CBG binding
            t7 = (- v['konC'] * (1/v['MWCBG']) * (x[:,22]/v['v_p']) * (x[:,2]/v['v_p'])
            * v['mictonano']
            # CBG off binding
            + v['koffC'] * (x[:,23]/v['v_p']) * (v['MWCort']/v['MWCC'])) * v['v_p']
            s = t1 + t2 + t3 + t4 + t5 + t6    
            arr[:,0] = s; arr[:,1] = t1; arr[:,2] = t2; arr[:,3] = t3
            arr[:,4] = t4; arr[:,5] = t5; arr[:,6] = t6; arr[:,7] = t7
        ...
        # Plot the system
        def plot_sols(t, sols, ts, data):
            """ 
            Plot mutliple sols of the hpa system
            """
            fig, ax = DEM.plt.subplots(4,4, figsize=(9,7), sharex=True)
            
            ax[0,0].plot(t, sols[:,0])
            ax[0,0].set_title('CRH (pg/ml)')
        
            ax[0,1].plot(t, sols[:,1])
            ax[0,1].plot(ts, data[:,0], 'o')
        
            ax[0,1].set_title('ACTH (pg/ml)')        
        
            ax[0,2].plot(t, (sols[:,2]))

            ax[0,2].set_title('CortP (ng/ml)')

            
            ax[0,3].plot(t, sols[:,3])
            ax[0,3].set_title('CortI (ng/ml)')       
            
            ax[1,0].plot(t, sols[:,4])
            ax[1,0].set_title('CortK (ng/ml)')
        
            ax[1,1].plot(t, sols[:,5])
            ax[1,1].set_title('CortisoneP (ng/ml)')       
            
            ax[1,2].plot(t, sols[:,7] + sols[:,9] + sols[:,11])
            ax[1,2].set_ylim([0.02, 0.06])
            ax[1,2].set_title('AlbI (g/ml)')      
        
            ax[1,3].plot(t, sols[:,8] + sols[:,10] + sols[:,12])
            ax[1,3].set_title('AlbCortI (ng/ml)')
        
            ax[2,0].plot(t, sols[:,14])
            ax[2,0].set_ylim([0.02, 0.06])
            ax[2,0].set_title('AlbP (g/ml)')       
        
            ax[2,1].plot(t, sols[:,15])
            ax[2,1].set_title('AlbCortP (ng/ml)')
        
            ax[2,2].plot(t, sols[:,16] + sols[:,18] + sols[:,20])
            ax[2,2].set_title('CBGI (ug/ml)')       
        
            ax[2,3].plot(t, sols[:,17] + sols[:,19] + sols[:,21])
            ax[2,3].set_title('CBGCortI (ng/ml)')
        
            ax[3,0].plot(t, sols[:,22])
            ax[3,0].set_title('CBGP (ng/ml)')       
        
            ax[3,1].plot(t, sols[:,23])
            ax[3,1].set_title('CBGCortP (ng/ml)')
        
            ax[3,2].plot(t, sols[:,13])
            ax[3,2].set_ylim([-2, 2])
            ax[3,2].set_title('COPE')    
        
            ax[3,3].plot(t, sols[:,6]) 
            ax[3,3].set_title('Cort T (ug/dl)')
            ax[3,3].plot(ts, data[:,1], 'o')           

            fig.tight_layout()
            return fig


    ConcSols = calc_conc(z, male_res.params.valuesdict())
    ConcSols[:,6] = toCortT_ugdl(z, male_res.params)
    male = plot_sols(ts2, ConcSols, ts, gall_male)
    plt.show(block=True)


    dervs = cort_p2(0, z, male_res.params.valuesdict())
    print()
    print('{:<8} {:<8} {:<8} {:<8} {:<8} {:<8} {:<8} {:<8}'.format( 
        'Sum', 'T1', 'T2', 'T3', 'T4', 'T5' , 'T6', 'T7'))
    numstr = '{:<8.2f} {:<8.2f} {:<8.2f} {:<8.2f} {:<8.2f} {:<8.2f} {:<8.2f} {:<8.2f}'
    for i in range(dervs.shape[0]):
        print(numstr.format(*dervs[i,:]))
